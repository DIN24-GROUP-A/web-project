...
router.get("/dashboard", isLoggedIn, (req, res) => {
	...
	if (user.is_admin) {
		...
		db.query("SELECT id, name, email, is_admin FROM users", (err, results) => {
			...
			db.query(
				"SELECT feedback.id, users.name AS user_name, feedback.topic, feedback.message, feedback.isABug, feedback.resolved, feedback.created_at FROM feedback JOIN users ON feedback.user_id = users.id ORDER BY feedback.created_at DESC",
				(err, feedbackResults) => {
					if (err) {
						console.error(err);
